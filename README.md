<div align="center">

# <img src="https://raw.githubusercontent.com/khs990704/studiary/main/frontend/favicon.svg" width="36" height="36" align="center" alt="Studiary logo"> Studiary

**집중을 기록하고, 성장을 확인하세요.**

하루 공부 세션을 기록하고, 집중도·방해요소 데이터를 바탕으로<br>AI가 요약과 피드백을 자동으로 생성해드립니다.

<br>

[![Live](https://img.shields.io/badge/🌐_Live-studiary.me-4ade80?style=for-the-badge)](https://studiary.me)
[![React](https://img.shields.io/badge/React-18-61DAFB?style=for-the-badge&logo=react&logoColor=black)](https://react.dev)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.115-009688?style=for-the-badge&logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com)
[![License](https://img.shields.io/badge/License-MIT-yellow?style=for-the-badge)](LICENSE)

</div>

---

## ✨ 주요 기능

| 기능 | 설명 |
|------|------|
| 🌿 **월별 히트맵** | 날짜별 집중도를 색상으로 시각화 |
| ⏱️ **세션 기록** | 타이머 기반 공부/휴식 세션, 집중도(1~5) 및 방해요소 입력 |
| 🤖 **AI 요약 & 피드백** | OpenRouter 무료 모델로 하루 학습 요약 및 개선 제안 자동 생성 |
| 📊 **과거 기록 조회** | 날짜별 세션 목록, 집중도 변화 그래프, AI 리뷰 확인 |
| 🔐 **계정 기반 인증** | Google OAuth 2.0 로그인, JWT 인증, 다중 기기 접속 |

---

## 🛠️ 기술 스택

<table>
  <tr>
    <td align="center"><b>Frontend</b></td>
    <td>React 18 · TypeScript · Vite 5 · Zustand · Tailwind CSS 3 · Recharts · Axios</td>
  </tr>
  <tr>
    <td align="center"><b>Backend</b></td>
    <td>FastAPI · Python 3.11 · SQLAlchemy 2.0 (async) · Alembic</td>
  </tr>
  <tr>
    <td align="center"><b>Database</b></td>
    <td>PostgreSQL 15</td>
  </tr>
  <tr>
    <td align="center"><b>AI</b></td>
    <td>OpenRouter API (llama-3.1-8b / gemma-3-4b / gemma-3-1b fallback)</td>
  </tr>
  <tr>
    <td align="center"><b>Infra</b></td>
    <td>Docker Compose · Nginx · Cloudflare (HTTPS) · OCI</td>
  </tr>
</table>

---

## 🏗️ 아키텍처

```
Browser → Cloudflare (HTTPS) → Nginx (:80) ─┬→ Frontend (:3000)
                                              └→ Backend  (:8200) → PostgreSQL (:5433)
                                                                   → OpenRouter API
```

---

## 📁 프로젝트 구조

```
studiary/
├── frontend/                # React SPA
│   └── src/
│       ├── api/             # Axios API 호출
│       ├── components/
│       │   ├── common/      # 공통 컴포넌트 (LoadingSpinner 등)
│       │   ├── heatmap/     # HeatmapGrid, HeatmapCell, MonthSelector
│       │   ├── layout/      # 레이아웃 컴포넌트
│       │   ├── review/      # AI 리뷰 컴포넌트
│       │   ├── session/     # 세션 입력/목록 컴포넌트
│       │   └── study-day/   # StudyDayCard 등
│       ├── hooks/           # useTimer, useAuth
│       ├── pages/           # LoginPage, RegisterPage, MainPage, StudyPage
│       ├── stores/          # Zustand (authStore, sessionStore, studyDayStore)
│       ├── types/           # TypeScript 타입 정의
│       └── utils/           # 날짜, 집중도, 상수
├── backend/                 # FastAPI 서버
│   └── app/
│       ├── models/          # SQLAlchemy 모델 (users, study_days, sessions, ai_results)
│       ├── schemas/         # Pydantic 스키마
│       ├── routers/         # auth, sessions, study_days, heatmap
│       ├── services/        # 비즈니스 로직
│       └── utils/           # JWT, bcrypt, OpenRouter 클라이언트
├── nginx/nginx.conf         # 리버스 프록시 설정
├── spec/                    # 사전 기획 문서
├── _workspace/              # 설계 확정본 + 테스트 계획 + 리뷰 보고서
├── docker-compose.yml
└── .env.example
```

---

## 🚀 시작하기

### 사전 요구사항

- Docker 24+, Docker Compose v2+
- [OpenRouter API 키](https://openrouter.ai/) (무료 모델 사용 가능)

### Docker로 전체 스택 실행

```bash
# 1. 환경변수 설정
cp .env.example .env
# .env 파일을 열어 실제 값으로 수정 (JWT_SECRET_KEY, OPENROUTER_API_KEY 필수)

# 2. 빌드 & 기동
docker compose up -d --build

# 3. 브라우저에서 접속
# → http://localhost
```

### 로컬 개발 (DB만 Docker)

> `DATABASE_URL` 호스트는 컨테이너 내부(`db:5432`)가 아닌 호스트에서 접근 가능한 `localhost:5433`을 사용하세요.

```bash
# DB만 Docker로 기동
docker compose up -d db

# 백엔드
cd backend
python -m venv venv && source venv/bin/activate
pip install -r requirements.txt
export DATABASE_URL=postgresql+asyncpg://studiary:studiary@localhost:5433/studiary
alembic upgrade head
uvicorn app.main:app --reload --port 8200

# 프론트엔드 (별도 터미널)
cd ../frontend
npm install
npm run dev   # → http://localhost:5173  (Vite proxy → localhost:8200)
```

---

## 📡 API 엔드포인트

> Base URL: `/api/v1`

| Method | Path | 설명 |
|:------:|------|------|
| `GET` | `/auth/google` | Google OAuth 로그인 시작 |
| `GET` | `/auth/google/callback` | Google OAuth 콜백 (JWT 발급) |
| `GET` | `/auth/me` | 내 정보 조회 |
| `GET` | `/study-days?year&month` | 월별 학습 기록 목록 |
| `GET` | `/study-days/{date}` | 특정 날짜 상세 (세션 포함) |
| `POST` | `/study-days/{date}/finish` | 공부 종료 + AI 요약 생성 |
| `POST` | `/study-days/{date}/regenerate-ai` | AI 요약 재생성 |
| `POST` | `/sessions` | 세션 생성 |
| `PATCH` | `/sessions/{id}` | 세션 수정 (집중도/방해요소) |
| `DELETE` | `/sessions/{id}` | 세션 삭제 |
| `GET` | `/heatmap?year&month` | 월별 히트맵 데이터 |

---

## ⚙️ 환경변수

| 변수 | 필수 | 설명 |
|------|:----:|------|
| `DATABASE_URL` | ✅ | PostgreSQL 접속 URL (asyncpg) |
| `JWT_SECRET_KEY` | ✅ | JWT 서명 키 (최소 32자, `openssl rand -hex 32` 으로 생성) |
| `JWT_ALGORITHM` | — | JWT 알고리즘 (기본: `HS256`) |
| `ACCESS_TOKEN_EXPIRE_MINUTES` | — | 토큰 만료 시간 (기본: `1440`분) |
| `OPENROUTER_API_KEY` | ✅ | OpenRouter API 키 |
| `OPENROUTER_BASE_URL` | — | OpenRouter 엔드포인트 |
| `CORS_ORIGINS` | — | CORS 허용 도메인 (기본: `*`) |
| `POSTGRES_USER` | ✅ | DB 사용자 |
| `POSTGRES_PASSWORD` | ✅ | DB 비밀번호 |
| `POSTGRES_DB` | ✅ | DB 이름 |

---

## 📄 문서

| 문서 | 설명 |
|------|------|
| [`spec/01_prd.md`](spec/01_prd.md) | 제품 요구사항 정의서 |
| [`spec/02_architecture_preview.md`](spec/02_architecture_preview.md) | 아키텍처 초안 |
| [`spec/03_api_preview.md`](spec/03_api_preview.md) | API 초안 |
| [`spec/04_db_preview.md`](spec/04_db_preview.md) | DB 초안 + ERD |
| [`spec/05_wireframe.md`](spec/05_wireframe.md) | 화면 설계 |
| [`_workspace/02_api_spec.md`](_workspace/02_api_spec.md) | API 명세 (확정본) |
| [`_workspace/03_db_schema.md`](_workspace/03_db_schema.md) | DB 스키마 (확정본) |
| [`_workspace/05_deploy_guide.md`](_workspace/05_deploy_guide.md) | 배포 가이드 |

---

<div align="center">
  <sub>MIT License · made with ☕ by <a href="https://github.com/khs990704">khs990704</a></sub>
</div>
